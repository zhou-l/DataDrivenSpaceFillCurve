...
function [LT, visitOrder] = SFCOctTree(Vlvls, OT, blockSizeFactor, minBlockSize)
LT = [];
visitOrder = [];
Idim = size(Vlvls{1});
global Isize;
...
%     blkSize = blockbr - blocktl;
    octBlkSize = bsFactor * blkSize(1);
    octBlktl = bsFactor * (blocktl - [1 1 1]) + [1 1 1];
    octBlkbr = octBlktl + octBlkSize - [1 1 1];
...
global minBlkSize;%%NOTE: the minimum block size may be greater than [1 1 1]!
    epix = ceil(entryPix ./ (2^(destLvl-srcLvl)*minBlkSize)); %ceil(entryPix ./ (2^(destLvl-srcLvl)*minBlkSize));
%     if destLvl == 1 && srcLvl >= 1
%         epix = (epix -[1 1 1]) ./ minBlkSize + [1 1 1];
%     end
end

function blkSize = lvlToNodeSize(lvl)
global minBlkSize;
   blkSize = 2^(lvl-1)*minBlkSize;
end
...
function lvl = nodeSizeToLvl(nodeSize)
   global minBlkSize;
   lvl = log2(nodeSize/minBlkSize) + 1;
end
